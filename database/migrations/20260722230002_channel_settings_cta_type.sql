...
-- channel_settings.cta_style already existed (Step 3) as free descriptive
-- text (e.g. "subscribe for weekly deep dives") — good for a human, not
-- something a script-generation prompt can branch on. Script generation
-- needs a fixed CTA *type* to script only the channel's actually
-- configured goal (never a generic "like and subscribe" by default, and
-- never an unconfigured monetization offer) — see
-- docs/architecture/script-pipeline.md#cta. cta_style is kept as-is,
-- unchanged, for the descriptive/tone guidance it already provides.
ALTER TABLE channel_settings ADD COLUMN cta_type TEXT
  CHECK (cta_type IS NULL OR cta_type IN ('subscribe', 'comment', 'affiliate_link', 'newsletter', 'next_video', 'product', 'community'));

-- Re-published to add cta_type into the returned `style` object — every
-- other field/behavior is unchanged from 20260722200000_workflow_runtime_functions.sql.
...
END;
$$ LANGUAGE plpgsql;
ALTER TABLE channel_settings DROP COLUMN IF EXISTS cta_type;
