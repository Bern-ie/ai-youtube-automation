...
Status: **implemented (Steps 6–7 — research, script generation).** Base
prompt templates common to all channels, parameterized by channel
configuration (tone, hook style, CTA style, language, content pillars,
allowed/blocked topics, etc.). See
...
`script-revision`) — the canonical, versioned rows for both live in
...
TTS and other later-stage prompts are not implemented yet.
